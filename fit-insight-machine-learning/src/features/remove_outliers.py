...
# Choose method and apply outliers removal
outliers_removed_df = df.copy()

for col in outlier_columns:
    for label in df["label"].unique():
        dataset = mark_outliers_chauvenet(df[df["label"] == label], col)
        # Replace values mark as outliers with nan
        dataset.loc[dataset[col + "_outlier"], col] = np.nan

        # Update the column in the original dataframe
        outliers_removed_df.loc[(outliers_removed_df["label"] == label), col] = dataset[col]

        outliers_n = len(dataset) - len(dataset[col].dropna())
        #print(f"Removed {outliers_n} from {col} for {label}")


outliers_removed_df.info()
outliers_removed_df.to_pickle("../../data/intermediate/02_outliers_removed_chauvenets.pkl")